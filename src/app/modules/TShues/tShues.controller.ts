...
import { ShoesService } from "./tSHues.service";
...
const getShoeByFilterC = catchAsync(async (req, res) => {
  ...
  const filters: Partial<TshoesInterface> = req.query;
...
const getSinglShoesData = catchAsync(async (req, res) => {
  const { _id } = req.params;
  const result = await ShoesService.getSingleShoesIntoDb(_id);
